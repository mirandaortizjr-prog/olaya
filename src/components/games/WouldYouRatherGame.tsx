...
import { getWouldYouRatherQuestions, calculateExperienceGain } from "@/lib/gameQuestions";
...
export const WouldYouRatherGame = ({ coupleId, userId, partnerId, onBack }: WouldYouRatherGameProps) => {
  ...
  const questions = getWouldYouRatherQuestions(progress.currentLevel, language);
  const currentQuestion = questions[currentQuestionIndex];

  useEffect(() => {
    checkExistingSession();
    const channel = supabase
      .channel('would-you-rather-game')
      .on('postgres_changes', {
        event: '*',
        schema: 'public',
  ...
  const createNewSession = async () => {
    ...
    const newSessionId = `would_you_rather_${Date.now()}`;
      ...
      <div className="fixed inset-0 bg-background z-50 flex flex-col p-4">
        ...
        <Card className="flex-1 p-6 overflow-auto">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-muted-foreground">{t.choicesMatched}</p>
          </div>
            ...
            {questions.map((q, index) => (
              <Card key={index} className="p-4">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    <p className="text-muted-foreground mb-1">{t.yourChoice}</p>
                    <p className="font-medium">{userAnswers[index]}</p>
                  </div>
                  <div>
                    <p className="text-muted-foreground mb-1">{t.partnerChoice}</p>
                    <p className="font-medium">{partnerAnswers[index]}</p>
                  </div>
                </div>
              </Card>
            ))}
  ...
  return (
    <div className="fixed inset-0 bg-background z-50 flex flex-col p-4">
      ...
      <Card className="flex-1 p-6">
        <h3 className="text-2xl font-bold mb-8 text-center">{t.wouldYouRather}</h3>
        ...
        <div className="space-y-4">
          {currentQuestion.options.map((option, index) => (
            <Button
              ...
              className="w-full text-left justify-start h-auto py-6 px-6"
